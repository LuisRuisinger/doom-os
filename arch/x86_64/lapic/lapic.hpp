...
{
    return (reg::isr_base as(u32) + (index * 0x10)) as(reg);
}

[[nodiscard]] inline constexpr reg tmr_register(usize index)
{
    return (reg::tmr_base as(u32) + (index * 0x10)) as(reg);
}
...
struct component
    : kernel::init::component<component, kernel::init::no_resource, kernel::acpi::component> {
...
struct core_component : kernel::init::component<core_component, kernel::init::no_resource,
    ...
    {
        static_cast<void>(dep<kernel::arch::x86_64::idt::core_component>(view));
        return init_core();
    }
};
